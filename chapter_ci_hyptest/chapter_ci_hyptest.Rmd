...
* Sous $H_0$, la statistique de test suit une ***distribution normale*** centrée en 0. La ***région de rejet*** (zone verte) correspond aux valeurs de la statistique de test jugées ***trop extrêmes*** pour être compatibles avec $H_0$.

--

* Sa taille est déterminée par le $\alpha$ : plus $\alpha$ est grand, plus la région de rejet est large.

--

* Si notre ***statistique observée*** tombe dans cette région, on ***rejette $H_0$***.
...
var xs = [];
for (var x=-4; x<=4.001; x+=0.05) xs.push(Math.round(x*100)/100);
var curve = xs.map(function(x){ return normPdf(x); });
...
var chart = new Chart(ctx, {
  ...
  options: {
    ...
    scales: {
      x: {
        ticks: { maxTicksLimit: 9, font: { size: 11 }, callback: function(val, idx){ var v = xs[idx]; return (Math.round(v*2)%2===0) ? v : ''; } },
        grid: { display: false },
        title: { display: true, text: 'Statistique de test', font: { size: 12 } }
      },
...
function update() {
  ...
  var fillData = xs.map(function(){ return 0; });
  var critLabel = '';

  if (side === 'right') {
    var zc = invNorm(1 - alpha);
    xs.forEach(function(x, i){ if (x >= zc) fillData[i] = curve[i]; });
    critLabel = zc.toFixed(2);
  } else if (side === 'left') {
    var zc2 = invNorm(alpha);
    xs.forEach(function(x, i){ if (x <= zc2) fillData[i] = curve[i]; });
    critLabel = zc2.toFixed(2);
  } else {
    var zc3 = invNorm(1 - alpha/2);
    xs.forEach(function(x, i){ if (x >= zc3 || x <= -zc3) fillData[i] = curve[i]; });
