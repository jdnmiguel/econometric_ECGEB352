...
1. Découvrez (en utilisant `help()` ou Google) comment importer un fichier .csv. N'utilisez PAS le bouton ``Import Dataset´´, ni n'installez de package.
...
1. Assurez-vous que `murders` est un data.frame en exécutant :
    ```{r,eval=F}
...
1. Visualisez le contenu de `murders` en cliquant sur `murders` dans votre espace de travail. À quoi correspond la variable `total` ?
